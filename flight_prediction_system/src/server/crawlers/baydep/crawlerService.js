...
import { delay, DELAY_MEDIUM, DELAY_LONG } from '../../utils/constants.js';
...
export async function runCrawler(page, { flightConfig, airports }) {
    ...
    const flightDateRaw = flightConfig.search_options.departure_date;
    const formattedFlightDate = flightDateRaw.replace(/\//g, '');
    const baydepcom_URL = `https://baydep.vn/flights/${flightConfig.departure_airport}${flightConfig.arrival_airport}${formattedFlightDate}/${flightConfig.adult}/${flightConfig.child}/${flightConfig.infant}?nearby-airport=yes`;


    console.log("============= Flight date="+flightDateRaw+"=================");
    ...
    try {
        console.log(`Target : ${baydepcom_URL}\n`);
...
// -------------------------------------------------------------------------------------------------------------------
        ...
        const searchResult = await performFlightSearch_BayDep(
            page, 
            departureAirport, 
            arrivalAirport, 
